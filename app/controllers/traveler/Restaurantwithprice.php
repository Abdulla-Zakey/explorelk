...
class Restaurant extends Controller
{
    ...
    public function index()
    {
        ...
        $this->view('traveler/restaurant', $data);
    }
    ...
    public function reserve()
    {
        ...
        if (!empty($missingFields)) {
            ...
            header('Location: ' . ROOT . '/traveler/restaurant?date=' . urlencode($data['date']) . '&error=Missing required fields: ' . urlencode(implode(', ', $missingFields)));
            exit;
        }
        ...
        if ($this->reservationModel->validate($data, $this->restaurant_id)) {
            ...
            if (!method_exists($this->reservationModel, 'getAllowedColumns')) {
                ...
                header('Location: ' . ROOT . '/traveler/restaurant?date=' . urlencode($data['date']) . '&error=Internal server error: Reservation model misconfigured');
                exit;
            }
            ...
            try {
                ...
                if ($result) {
                    ...
                    header('Location: ' . ROOT . '/traveler/restaurant?date=' . urlencode($data['date']) . '&success=Reservation created successfully');
                    exit;
                } else {
                    error_log("Failed to insert reservation: " . json_encode($filteredData));
                    header('Location: ' . ROOT . '/traveler/restaurant?date=' . urlencode($data['date']) . '&error=Failed to create reservation');
                    exit;
                }
            } catch (Exception $e) {
                error_log("Database insertion error: " . $e->getMessage());
                header('Location: ' . ROOT . '/traveler/restaurant?date=' . urlencode($data['date']) . '&error=Database error: ' . urlencode($e->getMessage()));
                exit;
            }
        } else {
            error_log("Validation failed with errors: " . json_encode($this->reservationModel->errors));
            header('Location: ' . ROOT . '/traveler/restaurant?date=' . urlencode($data['date']) . '&error=' . urlencode(json_encode($this->reservationModel->errors)));
            exit;
        }
    }
    ...
    public function cancelReservation()
    {
        ...
        if (empty($reservation_id)) {
            ...
            header('Location: ' . ROOT . '/traveler/restaurant?error=Missing reservation ID');
            exit;
        }
        ...
        if ($reservation) {
            ...
            try {
                ...
                if ($result !== false) {
                    ...
                    header('Location: ' . ROOT . '/traveler/restaurant?success=Reservation cancelled successfully');
                    exit;
                } else {
                    error_log("Failed to cancel reservation ID: {$reservation_id}");
                    header('Location: ' . ROOT . '/traveler/restaurant?error=Failed to cancel reservation');
                    exit;
                }
            } catch (Exception $e) {
                error_log("Database deletion error: " . $e->getMessage());
                header('Location: ' . ROOT . '/traveler/restaurant?error=Database error: ' . urlencode($e->getMessage()));
                exit;
            }
        } else {
            error_log("Reservation not found or does not belong to restaurant: {$reservation_id}");
            header('Location: ' . ROOT . '/traveler/restaurant?error=Invalid reservation');
            exit;
        }
    }
}
