    ...
    <div class="flexContainer">
        ...
        <div class="body-container">
            ...
            <form action="<?= ROOT ?>/tourGuide/C_addTour/add" method="POST" enctype="multipart/form-data">
                ...
                <input type="text" id="package-name" name="package-name" placeholder="Eg: Hiking in the tea fields"
                    value="<?= htmlspecialchars($data['package-name'] ?? '') ?>">

                <label for="tour-location">Tour Location</label>
                <input type="text" id="tour-location" name="tour-location" placeholder="Tour Location"
                    value="<?= htmlspecialchars($data['tour-location'] ?? '') ?>">

                <label for="duration">Duration</label>
                <input type="number" id="duration" name="duration" placeholder="No. of Days"
                    value="<?= htmlspecialchars($data['duration'] ?? '') ?>">

                <label for="number-of-people">Number of people</label>
                <input type="number" id="number-of-people" name="number-of-people" placeholder="No. of People"
                    value="<?= htmlspecialchars($data['number-of-people'] ?? '') ?>">

                <label for="rate">Rate</label>
                <input type="number" id="rate" name="rate" placeholder="Amount for your package"
                    value="<?= htmlspecialchars($data['rate'] ?? '') ?>">

                <label for="description">Add a detailed description</label>
                <textarea id="description" name="description" rows="5"
                    placeholder="Enter package details here"><?= htmlspecialchars($data['description'] ?? '') ?></textarea>

                <label for="activity-name">Add an activity or adventure</label>
                <div id="activities-container">
                    <input type="text" class="activity-input" name="activity-name" placeholder="Activity name"
                        value="<?= htmlspecialchars($data['activity-name-1'] ?? '') ?>">
                </div>

                <label for="upload-images">Upload an image</label>
                <div class="upload-container" id="upload-area">
                    <img src="<?= ROOT ?>/assets/images/tourGuide/upload-image.svg" alt="Upload Icon"
                        class="upload-icon">
